...
//
// D2Q4 scalar advection with DIAGONAL lattice velocities {(1,1),(-1,1),(-1,-1),(1,-1)},
// validating that the column-form stream of the schemes/lbm module transports diagonal
// velocities (and, at coarse levels, |c| > 1 donors) correctly. Same physics as
// new_D2Q4_advection but on the 45-degree-rotated velocity set.
//
// Moments : m0 = f0+f1+f2+f3               (conserved, u)
//           m1 = lambda (f0 - f1 - f2 + f3) (x-flux)
//           m2 = lambda (f0 + f1 - f2 - f3) (y-flux)
//           m3 = lambda^2 (f0 - f1 + f2 - f3) (cross)
...
{
    auto& app = samurai::initialize("D2Q4 diagonal-velocity scalar advection (schemes/lbm)", argc, argv);
    ...
    std::string filename  = "new_D2Q4diag_advection";
