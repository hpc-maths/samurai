...
void one_time_step(Field &f, Field &new_f, Func&& update_bc_for_level,
                                         ...
                                         const double sq_e, const double sxy_e)
{
    ...
    samurai::for_each_interval(mesh[mesh_id_t::cells], [&](std::size_t level, const auto& i, const auto& index)
    {
        ...
        auto m0_3 = xt::eval(lambda*lambda * (new_f(0, level, i, j) - new_f(1, level, i, j) + new_f(2, level, i, j) - new_f(3,  level, i, j)));
        ...
        auto m1_3 = xt::eval(lambda*lambda * (new_f(4, level, i, j) - new_f(5, level, i, j) + new_f(6, level, i, j) - new_f(7,  level, i, j)));
        ...
        auto m2_3 = xt::eval(lambda*lambda * (new_f(8, level, i, j) - new_f(9, level, i, j) + new_f(10, level, i, j) - new_f(11,  level, i, j)));
        ...
        auto m3_3 = xt::eval(lambda*lambda * (new_f(12, level, i, j) - new_f(13, level, i, j) + new_f(14, level, i, j) - new_f(15,  level, i, j)));
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        else
        {
            ...
            auto duration = toc();
            std::cout << "nb_ite: " << N << " execution time: " << duration << " MLUPS: " << N*(1<<level)*(1<<level)/duration/1e6 << std::endl;
        }
    }
