...
auto init_f(samurai::MRMesh<Config> &mesh,
            const double lambda = 5., const double gas_constant = 1.4,
            const double rho_up = 2., const double rho_down = 1.,
            const double p_up = 1., const double grav = 2.)
{
    ...
    samurai::for_each_cell(mesh[mesh_id_t::cells], [&](auto &cell) {
        ...
        if (y < 0.5 + .01*std::cos(8.*M_PI*x))  {
            p += (1. - (0.5 + .01*std::cos(4.*M_PI*x))) * grav * rho_up + (0.5 + .01*std::cos(4.*M_PI*x) - y) * grav * rho_down;
        }
        else
        {
            p +=  (1. - y) * grav * rho_up;
        ...
        double m1_1 =     (3./2. - gas_constant/2.) * (m1_0*m1_0)/(m0_0)
        ...
        double m2_2 =     (3./2. - gas_constant/2.) * (m2_0*m2_0)/(m0_0)
...
std::array<double, 5> vertical_boundary_condition(const bool up = true, const double gas_constant = 1.4, const double grav = 2., double lambda = 5.)
{
...
void one_time_step(Field &f,Func&& update_bc_for_level, const pred& pred_coeff,
                    ...
                    const double s_rho_x = 1.75, const double s_rho_xy = 1.,
                    const double s_u_x   = 1.5,  const double s_u_xy   = 1.,
                    const double s_p_x   = 1.5,  const double s_p_xy   = 1.,
                    const double grav = 2.)
{
    ...
    for (std::size_t level = min_level; level <= max_level; ++level)
    {
        ...
        if (level == max_level) { // Advection at the finest level
            ...
            leaves_east([&](auto &interval, auto& index) {
            ...
            leaves_west([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            leaves_south([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            leaves_north([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            leaves_north_east([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            leaves_north_west([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            leaves_south_east([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            leaves_south_west([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            auto diagonals = samurai::union_(samurai::union_(samurai::union_(leaves_north_east, leaves_north_west), leaves_south_east), leaves_south_west);

            auto remaining_leaves = samurai::difference(samurai::difference(samurai::difference(leaves, along_x), along_y), diagonals);

            remaining_leaves([&](auto& interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0]; // Logical index in y
                ...
                for (int scheme_n = 0; scheme_n < 4; ++scheme_n)    {
                    for (auto fl_nb : vld_flx)   {
                        for(auto &c: pred_coeff[j][fl_nb].coeff)
                        {
                            ...
                            fluxes((fl_nb>>1) + 4 * scheme_n + 1, lev_p_1, k, h) -= c.second * f((fl_nb>>1) + 4 * scheme_n + 1, lev_p_1, k + stencil_x, h + stencil_y);
                        }
                    }
                }
            });
            ...
            ol_northeast.on(lev_p_1)([&](auto& interval, auto& index) {
                ...
                for (int scheme_n = 0; scheme_n < 4; ++scheme_n)    {
                    ...
                    if (scheme_n == 0)  {
                        ...
                        // fluxes(3 + 4 * scheme_n + 1, lev_p_1, k, h) += (1<<j) * up_bc[4];
                    }
                    else
                    {
        ...
        leaves([&](auto& interval, auto& index) {
            ...
            auto m0_0 = xt::eval(                       advected(0, level, k, h)    + advected(1, level, k, h)    + advected(2, level, k, h)    + advected(3, level, k, h )   + advected(4,  level, k, h ));
            ...
            auto m0_3 = xt::eval(lambda*lambda/5. * (-4*advected(0, level, k, h) + 21*advected(1, level, k, h) + 21*advected(2, level, k, h) + 21*advected(3, level, k, h) + 21*advected(4,  level, k, h)));
            auto m0_4 = xt::eval(lambda*lambda * (                                    advected(1, level, k, h) -    advected(2, level, k, h) +    advected(3, level, k, h)    - advected(4,  level, k, h)));

            auto m1_0 = xt::eval(                 advected(5, level, k, h) + advected(6, level, k, h) + advected(7, level, k, h) + advected(8,  level, k, h ));
            ...
            auto m1_3 = xt::eval(lambda*lambda * (advected(5, level, k, h) - advected(6, level, k, h) + advected(7, level, k, h) - advected(8,  level, k, h)));

            auto m2_0 = xt::eval(                 advected(9, level, k, h) + advected(10, level, k, h) + advected(11, level, k, h) + advected(12,  level, k, h ));
            ...
            auto m2_3 = xt::eval(lambda*lambda * (advected(9, level, k, h) - advected(10, level, k, h) + advected(11, level, k, h) - advected(12,  level, k, h)));

            auto m3_0 = xt::eval(                 advected(13, level, k, h) + advected(14, level, k, h) + advected(15, level, k, h) + advected(16,  level, k, h ));
            ...
            auto m3_3 = xt::eval(lambda*lambda * (advected(13, level, k, h) - advected(14, level, k, h) + advected(15, level, k, h) - advected(16,  level, k, h)));
            ...
            m1_1 = (1 - s_u_x) *  m1_1 + s_u_x * ((3./2. - gas_constant/2.) * (m1_0*m1_0)/(m0_0) + (1./2. - gas_constant/2.) * (m2_0*m2_0)/(m0_0) + (gas_constant - 1.) * m3_0);
            ...
            new_f(1, level, k, h) = 1. /25 * m0_0 + .5/lambda * m0_1                        + 1./(20*lambda*lambda) * m0_3 + .25/(lambda*lambda) * m0_4;
            new_f(2, level, k, h) = 1. /25 * m0_0 +                   + .5/lambda * m0_2    + 1./(20*lambda*lambda) * m0_3 - .25/(lambda*lambda) * m0_4;
            new_f(3, level, k, h) = 1. /25 * m0_0 - .5/lambda * m0_1                        + 1./(20*lambda*lambda) * m0_3 + .25/(lambda*lambda) * m0_4;
            new_f(4, level, k, h) = 1. /25 * m0_0 +                   - .5/lambda * m0_2    + 1./(20*lambda*lambda) * m0_3 - .25/(lambda*lambda) * m0_4;
...
void save_solution(Field &f, double eps, std::size_t ite, const double gas_constant, std::string ext="")
{
    ...
    str << "LBM_D2Q5444_RayleighTaylor_" << ext << "_lmin_" << min_level << "_lmax-" << max_level << "_eps-"
        << eps << "_ite-" << ite;
        ...
        else
        {
            ...
            for (std::size_t nb_ite = 0; nb_ite <= N; ++nb_ite)
            {
                ...
                if (nb_ite % howoften == 0)    {
                    save_solution(f, eps, nb_ite/howoften, gas_constant); // Before applying the scheme
                }
