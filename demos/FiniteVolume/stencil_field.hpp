...
namespace samurai
{
        ...
        inline auto operator()(Dim<2> d, const Field& phi, const Field& phi_0, const std::size_t max_level) const
        {
            ...
            // auto out = xt::empty<double>({i.size()});
                ...
                xt::masked_view(out, mask) = xt::sqrt(xt::maximum(xt::pow(pos_part(dxp), 2.), xt::pow(neg_part(dxm), 2.))
                                                        +xt::maximum(xt::pow(pos_part(dyp), 2.), xt::pow(neg_part(dym), 2.))) - 1.;
    ...
    //
        ...
        {
            // auto vel_at_interface = xt::eval(.5 * (vel(0, level, i-1, j) + vel(0, level, i, j)));
        ...
        {
            // auto vel_at_interface = xt::eval(.5 * (vel(1, level, i, j-1) + vel(1, level, i, j)));
            auto vel_at_interface = xt::eval(3./8 * vel(1, level, i, j-1)
                                            +3./4 * vel(1, level, i, j  )
        ...
        {
            // auto vel_at_interface = xt::eval(.5 * (vel(1, level, i, j) + vel(1, level, i, j+1)));
            auto vel_at_interface = xt::eval(3./8 * vel(1, level, i, j+1)
                                            +3./4 * vel(1, level, i, j  )
                                            -1./8 * vel(1, level, i, j-1));
