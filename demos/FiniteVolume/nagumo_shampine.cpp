...
int main(int argc, char* argv[])
{
    ...
    // Fields
    auto u_s    = samurai::make_field<1>("u_s", mesh);
    auto f_D_U  = samurai::make_field<1>("f_D_U", mesh);
    auto u_tmp  = samurai::make_field<1>("u_tmp", mesh);
    auto u_tmp2 = samurai::make_field<1>("u_tmp2", mesh);

    // Matrix assembly
    auto J_R_op = id - dt * react;

    auto assembly = make_assembly(J_R_op);
    assembly.set_unknown(u_s);
    Mat J_R;
    ...
    // Solve the system
    Vec f_D_U_petsc = create_petsc_vector_from(f_D_U);
    Vec u_tmp_petsc = create_petsc_vector_from(u_tmp);
    ...
    if (l == 2)
    {
        Vec u_tmp2_petsc = create_petsc_vector_from(u_tmp2);
        KSPSolve(ksp, u_tmp2_petsc, u_tmp_petsc);
    }
