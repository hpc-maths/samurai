...
public:
    ...
    Laplacian1D(const Mesh& m, bool eliminate_dirichlet_values) :
        mesh(m)
    {
        _eliminate_dirichlet_values = eliminate_dirichlet_values;
    }

    static Laplacian1D create_coarse(const Laplacian1D& fine, const Mesh& coarse_mesh)
    {
        return Laplacian1D(coarse_mesh, fine._eliminate_dirichlet_values);
    }

private:
    //template<class Mesh>
    std::vector<int> preallocate_matrix()
    {
...
public:
    void create_matrix(Mat& A)
    {
        ...
        MatSeqAIJSetPreallocation(A, 5, preallocate_matrix().data());
        // ierr = MatSetOption(A, MAT_NEW_NONZERO_ALLOCATION_ERR, PETSC_FALSE);CHKERRQ(ierr);

        //std::cout << "n_cells -> " << mesh.nb_cells() << " " << mesh.nb_cells(mesh_id_t::cells_and_ghosts) << std::endl;
        //tic();
    }

    void create_and_assemble_matrix(Mat& A)
    {
        create_matrix(A);
        assemble_matrix(A);
    }
    ...
    PetscErrorCode assemble_matrix(Mat& A)
    {
        ...
        //std::cout << "assembling " << toc() << std::endl;
