...
enum class Case : int { abs, exp, tanh };
...
int main(int argc, char *argv[])
{
    ...
    std::map<std::string, Case> map{{"abs", Case::abs}, {"exp", Case::exp}, {"tanh", Case::tanh}};
    ...
    samurai::for_each_interval(u_reconstruct.mesh(), [&](std::size_t level, auto& i, auto& index)
    {
        auto j = index[0];
