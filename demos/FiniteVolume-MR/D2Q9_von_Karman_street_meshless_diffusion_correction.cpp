                ...
                double rho0, double u0, double lambda, double mu, double zeta, double radius, std::string & momenti)
{
    ...
    for (std::size_t level = min_level; level <= max_level; ++level)
    {
        ...
        else
        {
            ...
            touching_west.on(level + 1)([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            overleaves_west.on(level + 1)([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            overleaves_northwest.on(level + 1)([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            overleaves_southwest.on(level + 1)([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            touching_east.on(level + 1)([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            overleaves_east.on(level + 1)([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            overleaves_northeast.on(level + 1)([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            overleaves_southeast.on(level + 1)([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            overleaves_north.on(level + 1)([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            overleaves_south.on(level + 1)([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            // once for all
            auto all_overleaves = intersection(mesh[mesh_id_t::cells][level],
                                               mesh[mesh_id_t::cells][level]).on(level + 1);

            all_overleaves([&](auto &interval, auto& index) {
                auto k = interval; // Logical index in x
                auto h = index[0];    // Logical index in y
            ...
            leaves([&](auto &interval, auto& index) {
                ...
                for (int pop = 1; pop < 9; ++pop)  {
                    advected(pop, level, k, h) = f(pop, level, k, h) + 0.25 * (fluxes(pop, level + 1, 2*k,     2*h)
                                                                             + fluxes(pop, level + 1, 2*k + 1, 2*h)
        ...
        leaves([&](auto &interval, auto& index) {
            ...
            if  (! momenti.compare(std::string("Geier")))    {
                ...
                auto m3 = xt::eval(l2*(                           advected(1, level, k, h) + advected(2, level, k, h) + advected(3, level, k, h) + advected(4, level, k, h) + 2*advected(5, level, k, h) + 2*advected(6, level, k, h) + 2*advected(7, level, k, h) + 2*advected(8, level, k, h)));
                auto m4 = xt::eval(l3*(                                                                                                                                         advected(5, level, k, h) -   advected(6, level, k, h) -   advected(7, level, k, h) +   advected(8, level, k, h)));
                auto m5 = xt::eval(l3*(                                                                                                                                         advected(5, level, k, h) +   advected(6, level, k, h) -   advected(7, level, k, h) -   advected(8, level, k, h)));
                auto m6 = xt::eval(l4*(                                                                                                                                         advected(5, level, k, h) +   advected(6, level, k, h) +   advected(7, level, k, h) +   advected(8, level, k, h)));
...
void save_solution(Field & f, double eps, std::size_t ite, double lambda, std::string ext="")
{
    ...
    str << "LBM_D2Q9_von_Karman_street_" << ext << "_lmin_" << min_level << "_lmax-" << max_level << "_eps-"
        << eps << "_ite-" << ite;
    ...
    samurai::for_each_cell(mesh[mesh_id_t::cells], [&](auto &cell) {
        ...
        vel_mod[cell] = std::sqrt(qx[cell] * qx[cell]
