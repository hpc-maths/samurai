...
namespace samurai
{
    ...
    */
    TEST(sfc, hilbert2D_getKey){
        ...
        ij = { 3 , 4 };
        ASSERT_EQ( hilbert.getKey<dim>( ij ), static_cast<SFC_key_t>( 31 ) );

        ij = { 3 , 5 };
        ASSERT_EQ( hilbert.getKey<dim>( ij ), static_cast<SFC_key_t>( 28 ) );

        ij = { 3, 6 };
        ASSERT_EQ( hilbert.getKey<dim>( ij ), static_cast<SFC_key_t>( 27 ) );
