...
TEST(SubdomainBBox, BasicComputation)
{
    ...
    // Create a simple 2D mesh [0,1] x [0,1]
    samurai::CellList<dim> cl;
    constexpr std::size_t level = 3;
    
    cl[level][{}].add_interval({0, 8});  // Full domain at level 3: 8 cells in x
    ...
    EXPECT_DOUBLE_EQ(bbox.bbox.max_corner()[1], 1.0);
    
    EXPECT_EQ(bbox.min_level, level);
    EXPECT_EQ(bbox.max_level, level);
    EXPECT_GT(bbox.num_cells, 0);
}
...
TEST(SubdomainBBox, IntersectionDetection)
{
    ...
    BBox bbox1(0, samurai::Box<double, 2>({0.0, 0.0}, {0.5, 1.0}), 3, 3, 100);
    BBox bbox2(1, samurai::Box<double, 2>({0.5, 0.0}, {1.0, 1.0}), 3, 3, 100);
    
    // They should be neighbors (they touch)
    EXPECT_TRUE(bbox1.could_be_neighbor(bbox2, 1.0));
    EXPECT_TRUE(bbox2.could_be_neighbor(bbox1, 1.0));
}
...
TEST(SubdomainBBox, CandidateReduction)
{
    ...
    for (std::size_t i = 0; i < grid_size; ++i)
    {
        for (std::size_t j = 0; j < grid_size; ++j)
        {
            ...
            all_bboxes.emplace_back(
                rank,
                samurai::Box<double, 2>({x_min, y_min}, {x_max, y_max}),
                3, 3, 100
            );
        }
    }
    ...
    {
        if (other.rank != test_rank && 
            my_bbox.could_be_neighbor(other, 2.0))
        {
            bbox_candidates++;
        }
    }
