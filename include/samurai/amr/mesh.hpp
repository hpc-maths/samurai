...
namespace samurai
{
    ...
    template <class Config>
    struct get_mesh_id<amr::Mesh<Config>>
    {
    ...
    namespace amr
    {
        ...
        class Mesh : public Mesh_base<Mesh<Config>, Config>
        {
          public:
            ...
            Mesh(const ca_type& ca, const config_t& config);
            Mesh(const Box<double, dim>& b, const config_t& config);
        ...
        template <class Config>
        SAMURAI_INLINE Mesh<Config>::Mesh(const cl_type& cl, const config_t& config)
            : base_type(cl, config)
        {
        }

        template <class Config>
        SAMURAI_INLINE Mesh<Config>::Mesh(const ca_type& ca, const config_t& config)
            : base_type(ca, config)
        {
        }

        template <class Config>
        SAMURAI_INLINE Mesh<Config>::Mesh(const Box<double, dim>& b, const config_t& config)
            : base_type(b, config)
        {
        }
        ...
        SAMURAI_INLINE void Mesh<Config>::update_sub_mesh_impl()
        {
            ...
            {
                auto expr = difference(
                                union_(intersection(this->cells()[mesh_id_t::cells_and_ghosts][level - 1], this->get_union()[level - 1]),
            ...
            for (std::size_t level = min_level; level <= max_level; ++level)
            {
                ...
                expr(
                    [&](const auto& interval, const auto& index_yz)
                    {
                        ...
                        static_nested_loop<dim - 1, -config_t::prediction_stencil_radius, config_t::prediction_stencil_radius + 1>(
                            [&](auto stencil)
                            {
                                auto index = xt::eval(index_yz + stencil);
                                lcl[index].add_interval({interval.start - config_t::prediction_stencil_radius,
