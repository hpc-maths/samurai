...
namespace samurai
{
    namespace amr
    {
        ...
        inline void Mesh<Config>::update_sub_mesh_impl()
        {
            ...
            {
                auto expr = difference(union_(intersection(this->m_cells[mesh_id_t::cells_and_ghosts][level - 1], this->m_union[level - 1]),
            ...
            {
                auto expr = intersection(this->m_cells[mesh_id_t::pred_cells][level], this->m_cells[mesh_id_t::pred_cells][level]).on(level - 1);
            ...
            {
                lcl_type lcl{level};
                auto expr = union_(this->m_cells[mesh_id_t::cells_and_ghosts][level], this->m_cells[mesh_id_t::proj_cells][level]);
                expr(
                    [&](const auto& interval, const auto& index_yz)
                    {
                        lcl[index_yz].add_interval(interval);
