...
class MemoryPool
{
  public:
    ...
    Pointer getPtr(const Distance offset)
    {
        assert(Size(offset) < m_totalSize);
        return m_memory + offset;
    }

    const_Pointer getPtr(const Distance offset) const
    {
        assert(Size(offset) < m_totalSize);
        return m_memory + offset;
    }

    Reference at(const Distance offset)
    {
        return *getPtr(offset);
    }

    const_Reference at(const Distance offset) const
    {
        return *getPtr(offset);
    }

    Reference operator[](const Distance offset)
    {
        return at(offset);
    }

    const_Reference operator[](const Distance offset) const
    {
        return at(offset);
    }
...
{
    assert(std::all_of(std::begin(m_chunks),
                       std::end(m_chunks),
...
{
    const ChunkIterator chunkIt = std::find_if(std::begin(m_chunks),
                                               std::end(m_chunks),
                                               [chunkSize](const Chunk& chunk) -> bool
                                               {
    ...
    else if (m_memory != nullptr)
    {
        ...
        auto usedChunks = m_chunks
                        | std::views::filter(
                              [](const Chunk& chunk)
                              {
                                  return not chunk.isFree;
                              });
        ...
        {
            m_chunks.push_back(
                {.offset = Distance(m_totalSize + chunkSize), .size = Size(newTotalSize - m_totalSize - chunkSize), .isFree = true});
        }
...
auto MemoryPool<T, DefaultAllocator>::freeChunk(const OffsetRange offsets) -> void
{
    const ChunkIterator chunkIt = std::find_if(std::begin(m_chunks),
                                               std::end(m_chunks),
                                               [&offsets](const Chunk& chunk) -> bool
                                               {
                                                   return (not chunk.isFree) and chunk.offset == offsets.first
                                                      and chunk.size == offsets.size();
                                               });
    ...
    if (chunkIt != std::end(m_chunks))
    {
        ...
        for (ChunkIterator it = std::begin(m_chunks); it != std::end(m_chunks); ++it)
        {
            if (it->isFree)
            {
                for (auto nextIt = std::next(it);
                     nextIt != std::end(m_chunks) and nextIt->isFree and it->offset + Distance(it->size) == nextIt->offset;
                     ++nextIt)
                {
                    it->size += nextIt->size;
        ...
        const ChunkIterator newEnd = std::remove_if(std::begin(m_chunks),
                                                    std::end(m_chunks),
                                                    [](const Chunk& chunk) -> bool
                                                    {
                                                        return chunk.size == 0;
