...
public:
    FixedCapacityArray() noexcept : m_end(begin()) {}
    ...
    T* begin() noexcept
    { 
       if constexpr (std::is_trivially_constructible<T>::value) { return m_core.data();                       } 
       else                                                     { return reinterpret_cast<T*>(m_core.data()); }
    }
    T* end() { return m_end; }
    ...
    {
        if constexpr (std::is_trivially_constructible<T>::value) { return m_core.data();                             } 
        else                                                     { return reinterpret_cast<const T*>(m_core.data()); }
    }
    ...
    std::ptrdiff_t ssize() const { return std::distance(begin(), end()); }
