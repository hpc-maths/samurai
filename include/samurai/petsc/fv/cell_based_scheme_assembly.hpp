    ...
    namespace petsc
    {
        ...
        class Assembly<Scheme, std::enable_if_t<is_CellBasedScheme_v<Scheme>>> : public FVSchemeAssembly<Scheme>
        {
          ...
          public:
            ...
            using local_matrix_t = CollapsMatrix<field_value_type, output_field_components, field_components>;
          ...
          protected:
            ...
            inline auto local_col_index(unsigned int cell_local_index, [[maybe_unused]] unsigned int field_j) const
            {
                ...
                {
                    return cell_local_index * field_components + field_j;
                }
            }
            ...
            inline auto local_row_index(unsigned int cell_local_index, [[maybe_unused]] unsigned int field_i) const
            {
                ...
                {
                    return cell_local_index * output_field_components + field_i;
                }
            }
          ...
          public:
            ...
            void sparsity_pattern_scheme(std::vector<PetscInt>& nnz) const override
            {
                ...
                {
                    PetscInt scheme_nnz_i = stencil_size * field_components;
            ...
            void assemble_scheme(Mat& A) override
            {
                ...
                scheme().for_each_stencil_and_coeffs(
                    ...
                    [&](const auto& cells, const auto& coeffs)
                    {
                        ...
                        std::array<PetscInt, cfg_t::stencil_size * output_field_components> rows;
                        ...
                        std::array<PetscInt, cfg_t::stencil_size * field_components> cols;
                        for (unsigned int c = 0; c < cfg_t::stencil_size; ++c)
                        {
                            for (unsigned int field_j = 0; field_j < field_components; ++field_j)
                            {
                        ...
                        if constexpr (field_components == 1 || field_t::is_soa)
                        {
                        ...
                        else // AOS
                        {
                            ...
                            {
                                // Insert a coefficient block of size <output_field_components x field_components>:
                                // - in 'rows', for each cell, <output_field_components> rows are contiguous.
                                // - in 'cols', for each cell, <field_components> cols are contiguous.
                                // - coeffs[c] is a row-major matrix (xtensor), as requested by PETSc.
                                MatSetValues(A,
                                             static_cast<PetscInt>(output_field_components),
                                             &rows[local_row_index(cfg_t::center_index, 0)],
                                             static_cast<PetscInt>(field_components),
