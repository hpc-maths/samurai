...
namespace Load_balancing
{
    ...
    class GlobalCriteria : public samurai::LoadBalancer<GlobalCriteria>
    {
      private:
        ...
        {
            double s_ = 0.;
            for_each_cell(mesh[Mesh_t::mesh_id_t::cells],
                          [&](const auto& cell)
                          {
                              double s = 1.;
      ...
      public:
        ...
        auto load_balance_impl(Mesh_t& mesh)
        {
            ...
            // Invalidate fluxes with non adjacents neighbours, should not happen or might happen if load balancing exchange does not
            // consider some direction.
            ...
            for_each_cell(mesh[Mesh_t::mesh_id_t::cells],
                          [&](const auto& cell)
                          {
                              ...
                              { // for each neighbour
                                  ...
                                  // double dist = samurai::getDistance<dim, fdist>( cell, barycenter_interface_neighbours[ ni ] ) / loads[
                                  // neighbour_rank ]; double coeff = sv[ ni ] / ( loads[ neighbour_rank ] ) ; // / sv[ ni ];
                              ...
                              // logs << fmt::format("\t\t\t> Cell given to process #{}", neighbourhood[ static_cast<size_t>( winner_id )
                              // ].rank ) << std::endl;
        ...
        _barycenterWithMeshWithFlags(const Mesh_t& mesh, const Field_t& flags, int rank, const Mesh_t& otherMesh)
        {
            using Coord_t = xt::xtensor_fixed<double, xt::xshape<dim>>;
            ...
            samurai::for_each_cell(otherMesh,
                                   [&](const auto& cell)
                                   {
                                       double wght = 1.;
        ...
        auto load_balance_impl2(Mesh_t& mesh)
        {
            ...
            // Invalidate fluxes with non adjacents neighbours, should not happen or might happen if load balancing exchange does not
            // consider some direction or neighbourdhood is overkill (all meshes for example)
            std::vector<size_t> ncells_interface(n_neighbours, 0);
            for (size_t ni = 0; ni < n_neighbours; ni++)
            {
                samurai::for_each_interval(
            ...
            for (size_t nbi = 0; nbi < n_neighbours; ++nbi)
            {
                barycenter_neighbours[nbi] = _barycenterWithMeshWithFlags<Mesh_t::dim>(mesh,
                                                                                       flags,
                                                                                       neighbourhood[nbi].rank,
                                                                                       neighbourhood[nbi].mesh);
            ...
            for (size_t nbi = 0; nbi < n_neighbours; ++nbi)
            {
                ...
                // debug
                auto s_ = fmt::format("Updated Barycenter neighbour # {}: ({}, {})",
                                      neighbourhood[nbi].rank,
                                      barycenter_neighbours[nbi](0),
                                      barycenter_neighbours[nbi](1));
