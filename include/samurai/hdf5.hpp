...
namespace samurai
{
    ...
    void save_mesh(const fs::path& path, const std::string& filename, const Mesh& mesh)
    {
        ...
        for (std::size_t level = min_level; level <= max_level; ++level)
        {
            ...
            {
                H5Easy::dump(file, fmt::format("/mesh/level/{}/intervals/{}", level, d), mesh[mesh_id_t::cells][level][d]);
            }

            for (std::size_t d = 1; d < dim; ++d)
            {
                H5Easy::dump(file, fmt::format("/mesh/level/{}/offsets/{}", level, d), mesh[mesh_id_t::cells][level].offsets(d));
            }
        }
    }
    ...
    ca_type load_mesh(const fs::path& path, const std::string& filename)
    {
        ...
        ca_type ca;
        HighFive::File file(fmt::format("{}_mesh.h5", (path / filename).string()), HighFive::File::ReadOnly);
        auto dim_file  = H5Easy::load<std::size_t>(file, "/mesh/dim");
