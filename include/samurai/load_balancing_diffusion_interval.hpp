...
class Diffusion_LoadBalancer_interval : public samurai::LoadBalancer<Diffusion_LoadBalancer_interval<dim>>
{
  ...
  private:
    ...
    Coord_t _cmpIntervalBarycenter(Mesh_t& mesh)
    ...
    inline Coord_t _getIntervalMidPoint(size_t level, const Interval_t& interval, const Index_t& index) const
    {
        Coord_t mid;
  ...
  public:
    ...
    Mesh_t load_balance_impl(Mesh_t& mesh)
    {
        ...
        using CellArray_t     = samurai::CellArray<Mesh_t::dim>;
        ...
        while (!balancing_done)
        {
            ...
            { // check emptyness of interface, if it is empty, then set fluxes for this neighbour to 0
                ...
                samurai::for_each_interval(
                    interface,
                    [&]([[maybe_unused]] std::size_t level, [[maybe_unused]] const auto& interval, [[maybe_unused]] const auto& index)
                    {
                        nelement += 1;
                    });
                ...
                if (nelement == 0)
                {
                    ...
                    { // debug
                        std::cerr << fmt::format("\t> Process {}, Warning no interface with a requester neighbour # {}",
                                                 world.rank(),
                                                 neighbourhood[requester].rank)
                                  << std::endl;
            ...
            samurai::for_each_interval(interface,
                                       [&](std::size_t level, const auto& interval, const auto& index)
                                       {
            ...
            { // remove interval from current process mesh and add it to neighbour mesh local copy !
                ...
                samurai::for_each_interval(ca_for_neighbour,
                                           [&](std::size_t level, const auto& interval, const auto& index)
                                           {
        ...
        for (std::size_t ni = 0; ni < n_neighbours; ++ni)
        {
            ...
            { // receive data
                ...
                logs << fmt::format("\t>[load_balance_impl]::interval Rank # {} receiving {} cells from rank # {}",
                                    world.rank(),
                                    to_rcv.nb_cells(),
                                    neighbourhood[ni].rank)
                     << std::endl;
            ...
            { // send data to
                ...
                logs << fmt::format("\t>[load_balance_impl]::interval Rank # {} sending {} cells to rank # {}",
                                    world.rank(),
                                    to_send.nb_cells(),
                                    neighbourhood[ni].rank)
                     << std::endl;
    ...
    void load_balance_impl2(Mesh_t& mesh)
    {
        ...
        auto interface = samurai::_computeCartesianInterface<Mesh_t::dim, samurai::Direction_t::FACE>(mesh);
        ...
        Coord_t barycenter = samurai::_cmpCellBarycenter<Mesh_t::dim>(mesh[mesh_id_t::cells]);
        ...
        for (size_t nbi = 0; nbi < n_neighbours; ++nbi)
        {
            ...
            barycenter_neighbours[nbi] = samurai::_cmpCellBarycenter<Mesh_t::dim>(neighbourhood[nbi].mesh[mesh_id_t::cells]);
        ...
        struct Data
        {
            size_t level;
            interval_t interval;
            xt::xtensor_fixed<samurai::default_config::value_t, xt::xshape<Mesh_t::dim - 1>> indices;
            int rank;
        };
        ...
        for_each_interval(mesh[Mesh_t::mesh_id_t::cells],
                          [&](std::size_t level, const auto& interval, const auto& index)
                          {
                              ...
                              double winner_dist = samurai::getDistance<Mesh_t::dim, fdist>(mid_point, barycenter) * invloads[_rank];
                              ...
                              { // for each neighbour
                                  ...
                                  if (fluxes[ni] >= 0)
                                  {
                                      continue; // skip neighbour that will recv
                                  }
                                  ...
                                  double dist = samurai::getDistance<Mesh_t::dim, fdist>(mid_point, barycenter_neighbours[ni])
                                              * invloads[neighbour_rank];
                                  // double dist = samurai::distance_inf<Mesh_t::dim>( mid_point, barycenter_interface_neighbours[ ni ] );
        ...
        for (auto it = repartition.begin(); it != repartition.end(); it++)
        {
            ...
            logs << fmt::format("\t> Interval {} --> to rank {} ( level : {} , dist : {}", it->second.interval, nrank, lvl, it->first)
                 << std::endl;
        ...
        for (size_t nbi = 0; nbi < n_neighbours; ++nbi)
        {
            ...
            logs << "\t\t> Number of cells to send to process # " << neighbourhood[nbi].rank << " : " << ca_to_send[nbi].nb_cells()
                 << std::endl;
        }
        ...
        for (size_t ni = 0; ni < n_neighbours; ++ni)
        {
            ...
            if (fluxes[ni] > 0)
            { // receive data
                samurai::CellArray<Mesh_t::dim> to_rcv;
                world.recv(neighbourhood[ni].rank, 42, to_rcv);
                mesh.merge(to_rcv);
            }
            else
