...
class SFC
{
  public:
    ...
    inline SFC_key_t getKey(const Coord_t& lc) const
    {
        if constexpr (dim == 2)
        {
            return static_cast<const SFC_Flavor*>(this)->getKey_2D_impl(lc);
        }
        if constexpr (dim == 3)
        {
            return static_cast<const SFC_Flavor*>(this)->getKey_3D_impl(lc);
        }
    }
    ...
    inline auto getCoordinates(const SFC_key_t& clef) const
    {
        if constexpr (dim == 2)
        {
            return static_cast<const SFC_Flavor*>(this)->getCoordinates_2D_impl(clef);
        }
        if constexpr (dim == 3)
        {
            return static_cast<const SFC_Flavor*>(this)->getCoordinates_3D_impl(clef);
        }
    }
