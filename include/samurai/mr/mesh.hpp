    ...
    inline void MRMesh<Config>::update_sub_mesh_impl()
    {
        ...
        if (this->max_level() != this->min_level())
        {
            ...
            for_each_level(
                ...
                {
                    // own part
                    add_prediction_ghosts(nestedExpand<config::prediction_order>(self(this->cells()[mesh_id_t::cells][level]).on(level - 2)),
                                          intersection(nestedExpand<config::prediction_order>(
                                                           self(this->cells()[mesh_id_t::cells_and_ghosts][level]).on(level - 1)),
                                                       nestedExpand<config::prediction_order>(self(this->subdomain()).on(level - 1))),
                                          level);
