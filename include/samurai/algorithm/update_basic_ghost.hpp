...
namespace samurai
{
    ...
    void update_ghost(Field& field, Fields&... fields)
    {
        ...
        // The outer ghosts of a level are written once every cell of the level inside the
        // domain has its value, i.e. after its prediction ghosts; the coarsest level has none.
        const std::size_t min_level = mesh[mesh_id_t::reference].min_level();
        if (min_level > 0)
        {
            update_outer_ghosts(min_level - 1, field, fields...);
        }
        for (std::size_t level = min_level; level <= max_level; ++level)
        {
            ...
            update_outer_ghosts(level, field, fields...);
        }
