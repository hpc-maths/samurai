...
namespace samurai
{
    ...
    project_bc(std::size_t proj_level, std::size_t data_level, const DirectionVector<Field::dim>& direction, int layer_width, Field& field)
    {
        ...
        //   auto& inner_proj_ghosts = intersection(mesh[mesh_id_t::cells][data_level + 1],
        //   mesh[mesh_id_t::reference][data_level]).on(data_level);

        // auto& inner_proj_ghosts = mesh.get_union()[data_level];
        //   auto inner_proj_ghosts = difference(mesh.get_union()[data_level], self(mesh.get_union()[data_level + 3]).on(data_level));
        //  auto inner_proj_ghosts = difference(mesh.get_union()[proj_level], self(mesh.get_union()[proj_level + 3]).on(proj_level));
        ...
        // auto inner_all     = union_(inner_proj_ghosts, cells);
        auto inner_all = mesh.get_union()[proj_level];
        // auto inner_all = difference(mesh.get_union()[proj_level], self(mesh.get_union()[proj_level + 2]).on(proj_level));
        //     auto inner_all     = mesh.get_union()[proj_level];
            ...
            projection_ghosts(
                [&](const auto& i, const auto& index)
                {
                    ...
                    for (auto ii = i.start; ii < i.end; ++ii)
                    {
                        ...
                        int n_children = 0;
                        // We retrieve the children of the current ghost by intersecting with the upper level
                        ...
                        children(
                            [&](const auto& i_child, const auto& index_child)
                            {
                                ...
                                {
                                    // std::cout << "field(" << proj_level << "," << i << "," << index << ") += field(" << proj_level + 1
                                    //           << "," << ii_child << "," << index_child << ")" << std::endl;
                                    ...
                                    {
                                        std::cout << "field(" << proj_level + 1 << "," << ii_child << "," << index_child
                                                  << ") = " << field(proj_level + 1, {ii_child, ii_child + 1}, index_child) << std::endl;
                                        std::exit(1);
                                    }
                                }
                            });
        ...
        for_each_cartesian_direction<dim>(
            [&](auto& direction)
            {
                ...
                if (!mesh.is_periodic(d))
                {
                    ...
                    {
                        // if (direction[0] == -1 && level == 5)
                        // {
                        //     samurai::save(fs::current_path(), fmt::format("update_ghosts"), {true, true}, field.mesh(), field);
                        // }
                        if (direction[0] == 1 && level == 4)
                        {
                            samurai::save(fs::current_path(), fmt::format("update_ghosts"), {true, true}, field.mesh(), field);
                        }
