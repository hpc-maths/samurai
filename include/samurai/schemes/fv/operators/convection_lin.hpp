    ...
    auto make_convection_weno5(const VelocityVector<Field::dim>& velocity)
    {
        ...
        static_for<0, dim>::apply( // for each positive Cartesian direction 'd'
            [&](auto integral_constant_d)
            {
                ...
                if (velocity(d) >= 0)
                {
                    weno5[d].cons_flux_function = [&velocity](auto& /*cells*/, const StencilValues<cfg>& u) -> FluxValue<cfg>
                    {
    ...
    auto make_convection_weno5(const VelocityField& velocity_field)
    {
        ...
        static_for<0, dim>::apply( // for each positive Cartesian direction 'd'
            [&](auto integral_constant_d)
            {
                ...
                weno5[d].cons_flux_function = [&velocity_field](auto& cells, const StencilValues<cfg>& u) -> FluxValue<cfg>
                {
