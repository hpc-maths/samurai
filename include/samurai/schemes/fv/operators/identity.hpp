...
namespace samurai
{
    ...
    auto make_identity()
    {
        ...
        using cfg = LocalCellSchemeConfig<SchemeType::LinearHomogeneous, Field::nb_components, Field>;

        auto identity = make_cell_based_scheme<cfg>("Identity");

        identity.coefficients_func() = [](double) -> StencilCoeffs<cfg>
        {
            // return {eye<field_value_type, field_components, field_components>()};
            StencilCoeffs<cfg> sc;
            sc(0) = eye<field_value_type, field_components, field_components>();
            return sc;
        };
