...
namespace samurai
{
    ...
    TEST(box, length)
    {
        Box<int, 2> box_1{{0, 0}, {10, 5}};
        xt::xarray<int> expected_1{10, 5};

        Box<int, 2> box_2{{-5, -10}, {10, 5}};
    ...
    TEST(box, is_valid)
    {
        EXPECT_TRUE((Box<int, 2>{{0, 0}, {1, 1}}).is_valid());
        EXPECT_FALSE((Box<int, 2>{{1, 1}, {0, 0}}).is_valid());
        EXPECT_FALSE((Box<int, 2>{{0, 0}, {1, 0}}).is_valid());
        EXPECT_FALSE((Box<int, 2>{{0, 0}, {0, 1}}).is_valid());
    }
