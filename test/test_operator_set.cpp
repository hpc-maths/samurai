...
namespace samurai
{
    ...
    auto give_me_lca_1d()
    {
        ...
        const auto level = *rc::gen::container<std::array<std::size_t, size>>(
            rc::gen::inRange(2, 12));
    ...
    RC_GTEST_PROP(operator, distributive_law_intersection_1d,())
    {
        ...
        auto expr2 =
            union_(intersection(lca[0], lca[1]), intersection(lca[0], lca[2]))
                .on(ref_level);
    ...
    RC_GTEST_PROP(operator, distributive_law_union_1d,())
    {
        ...
        auto expr2 =
            intersection(union_(lca[0], lca[1]), union_(lca[0], lca[2]))
                .on(ref_level);
    ...
    RC_GTEST_PROP(operator, de_morgan_law_intersection_1d,())
    {
        ...
        auto expr1 =
            difference(lca_c,
                       intersection(lca[0], intersection(lca[1], lca[2])))
                .on(ref_level);
        auto expr2 =
            union_(difference(lca_c, lca[0]),
                   union_(difference(lca_c, lca[1]), difference(lca_c, lca[2])))
                .on(ref_level);
    ...
    RC_GTEST_PROP(operator, de_morgan_law_union_1d,())
    {
        ...
        auto expr1 = difference(lca_c, union_(lca[0], union_(lca[1], lca[2])))
                         .on(ref_level);
        auto expr2 = intersection(difference(lca_c, lca[0]),
                                  intersection(difference(lca_c, lca[1]),
                                               difference(lca_c, lca[2])))
                         .on(ref_level);
