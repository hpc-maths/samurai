...
Formula :eq:`linear_comb` straightforwardly generalizes to a vectorial field :math:`\mathbf{u}_h` of size :code:`n_comp`:

.. math::
    \mathcal{F}_h(\mathbf{u}_h)_{|F} := \sum_i \mathbf{c}_i*\mathbf{u}_i, \qquad \text{ where } \mathbf{u}_i := (\mathbf{u}_h)_{|V_i}


:math:`(\mathbf{u}_i)_i` are now vectors of size :code:`n_comp` and :math:`(\mathbf{c}_i)_i` are now matrices of size :code:`n_comp`:math:`\times`:code:`n_comp`.
For :code:`n_comp = 2`, the same scheme as the scalar laplacian writes,
