...
static void BM_SetOPWithOn2(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        auto subset = samurai::intersection(samurai::intersection(set1, samurai::translate(set2, stencil)), samurai::translate(set3, stencil)).on(15);
        subset([&](auto& interval, auto&)
        {
            interval *= 2;
        });
...
static void BM_BigDomain(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        subset([&](const auto& interval, auto&)
        {
