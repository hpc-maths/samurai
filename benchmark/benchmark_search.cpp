...
auto generate_mesh(int bound, std::size_t start_level, std::size_t max_level)
{
    ...
    for (std::size_t ite = 0; ite < max_level - start_level; ++ite)
    {
        ...
        samurai::for_each_interval(
            ...
            {
                auto choice = xt::random::choice(
                    xt::xtensor_fixed<bool, xt::xshape<2>>{ true, false },
                    interval.size());
                for (int i = interval.start, ic = 0; i < interval.end;
                     ++i, ++ic)
                {
                    if (choice[ic])
                    {
                        samurai::static_nested_loop<dim - 1, 0, 2>(
                            [&](auto stencil)
                            {
                                ...
                                cl[level + 1][new_index].add_interval(
                                    { 2 * i, 2 * i + 2 });
                            });
                    }
                    else
...
class MyFixture : public ::benchmark::Fixture {
  public:
    ...
    MyFixture() { mesh = generate_mesh<dim_>(bound, min_level, max_level); }
    ...
    void bench(benchmark::State& state)
    {
        ...
        for (auto _ : state)
        {
            for (std::size_t s = 0; s < state.range(0); ++s)
            {
                ...
                for (auto& c : coord)
                {
                    c = std::experimental::randint(-bound << level,
                                                   (bound << level) - 1);
                }
        ...
        state.counters["found"] =
            static_cast<double>(found) / state.iterations();
    }
